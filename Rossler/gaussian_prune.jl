...
is_restart=true
is_prune=true
sparse_on=1 #set this to 1 and see reg_loss() and prune() functions if sparsity is desired 

# Directories
dir         = @__DIR__
dir         = dir*"/"
cd(dir)
fname       = "gaussian_sparse"
...
function prune(p, kan_curr, layer_width, grid_size, pM_axis, theta=1e-2)
    ...
    activations_x, activations_y, activations_z, activations_second = activation_getter_3x3(pM_, pM_new, kan_curr, grid_size)

    nodes_to_eval = 1:layer_width
    nodes_to_keep = []

    # Prune based on input and output scores
    for i in nodes_to_eval
     # Compute input score using x, y, z activations
        input_score = maximum([
            maximum(abs.(activations_x[i, :])),
            maximum(abs.(activations_y[i, :])),
            maximum(abs.(activations_z[i, :]))
           ])

    # Compute output score for second layer activations
        output_score = maximum(maximum(abs.(activations_second[(i-1)*3+1:(i-1)*3+3, :])))

    # Keep node if both input and output scores exceed threshold
        if minimum([input_score, output_score]) > theta
            append!(nodes_to_keep, i)
        end
    end

    # Reinitialize KAN with the smaller size
    ...
    # Save only the pruned parameters into the new model
    pm1c = pM.layer_1.C[nodes_to_keep, :]
    pm1w = pM.layer_1.W[nodes_to_keep, :]
    pm2c = zeros(3, grid_size * layer_width)
    count = 0
    ...
    pm2w = pM.layer_2.W[:, nodes_to_keep]

    pM_new = (
        layer_1 = (C = pm1c, W = pm1w),
        layer_2 = (C = pm2c, W = pm2w),
    )
...
basis_func = gaussian_basis_function      # rbf, rswaf
...
function plot_save(l, l_test, p_list, epoch)
    ...
    pred_sol_true = solve(ODEProblem(rossler!, u0, tspan, p_), Tsit5(), abstol = 1e-12, reltol = 1e-12, saveat = timestep)
    ...
    file = matopen(dir*add_path*"checkpoints/"*fname*"_prune_results.mat", "w")
