...
load_file=dir*add_path_kan*"checkpoints/"*fname*"_steady_results.mat"
load_file_mlp=dir*add_path_mlp*"checkpoints/"*fname_mlp*"_results_MLP_steady.mat"
...
xsort=sortpload_file=dir*add_path_kan*"checkpoints/"*fname*"_results_steady.mat"
load_file_mlp=dir*add_path_mlp*"checkpoints/"*fname_mlp*"_results_MLP_steady.mat"
...
plt=scatter(solution.t[1:end_index],reduce(hcat,solution.u)'[1:end_index, 1], margin=3Plots.mm, 
           legend=(0.6,0.97), alpha = 0.75, label = "Train x",ylims=(0,2.5),dpi=1000,size=(475, 200), grid=false, color=:mediumseagreen)
...
plot!(solution.t[end_index+1:end],reduce(hcat,solution.u)'[end_index+1:end, 1],  
label = "Test x", linestyle=:dot, color=:mediumseagreen, linewidth=3, thickness_scaling = 1)
plot!(solution.t[end_index+1:end], reduce(hcat,solution.u)'[end_index+1:end, 2],  
label = "Test y", linestyle=:dot, color=:cornflowerblue, linewidth=3, thickness_scaling = 1)
vline!([20], color=:darkorange1, label = "Train/test split", legend_columns=2, linewidth=2, thickness_scaling = 1, linestyle=:dot)
xlabel!("Time [s]")
ylabel!("x,y ")
png(plt, string(figpath, "/paper_reconstruction_rbf_50_not_sparse.png"))
...
xlabel!("Epoch")
ylabel!("Loss")
png(plt, string(figpath, "/loss_kan_rbf_50_not_sparse.png"))
...
xlabel!("Epoch")
ylabel!("Loss")
png(plt, string(figpath, "/loss_mlp_50.png"))
