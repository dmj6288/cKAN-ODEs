...
dir         = dir*"/"
cd(dir)
fname       = "rbf_steady_sparse_results"
...
function prune(p, kan_curr, layer_width, grid_size, pM_axis, theta=1e-2)
    ...
    #load current save
    load_file=dir*add_path*"checkpoints/"*fname*".mat"
...
if is_prune==true
    pM, kan1, layer_width=prune(p, kan1, layer_width, grid_size, pM_axis, 0.175)
end
...
function loss(p)
    ...
    if sparse_on==1
        loss_temp+=reg_loss(p, 1e-6, 0) #if we have sparsity enabled, add the reg loss
    end
    return loss_temp
end
...
function plot_save(l, l_test, p_list, epoch)
    ...
    println("minimum train loss: ", minimum(l), "\nminimum test loss: ", minimum(l_test))
    ...
    pred_sol_true = solve(ODEProblem(CSTR!, u0, tspan, p_), Tsit5(), abstol = 1e-12, reltol = 1e-12, saveat = timestep)
    plt=scatter(pred_sol_true, alpha = 0.75)
    plot!(pred_sol_kan)
    vline!([12.5], color=:black, label = "train/test split")
    xlabel!("Days")
    ylabel!("x, y")
    ...
    file = matopen(dir*add_path*"checkpoints/"*fname*"_prune0.175_steady_results.mat", "w")
