...
function CSTR!(du, u, p, t)
    ...
    S, X = u
#    α, β, γ, δ = p
    du[1] = 0.8 * (2 - S) - S * X/(0.2 + S)
    du[2] = S * X/(0.2 + S) - 0.8 * X
...
prune_level = 0.25
fname = "rbf_steady_sparse_results_prune$(prune_level)_steady_results"
...
#=
hallOfFame = EquationSearch(inputs_lay1[1, :]', activations_lay1_x[:, 1]', niterations=100, options=options)
hallOfFame = EquationSearch(inputs_lay1[1, :]', activations_lay1_x[:, 2]', niterations=100, options=options)

hallOfFame = EquationSearch(inputs_lay1[2, :]', activations_lay1_y[:, 1]', niterations=100, options=options)
hallOfFame = EquationSearch(inputs_lay1[2, :]', activations_lay1_y[:, 2]', niterations=100, options=options)
...
hallOfFame = EquationSearch(inputs_lay2[2, :]', activations_lay2[1, :]', niterations=100, options=options)
hallOfFame = EquationSearch(inputs_lay2[2, :]', activations_lay2[2, :]', niterations=100, options=options)
=#
...
#hallOfFame = EquationSearch(inputs_lay2[4, :]', activations_lay2[7, :]', niterations=100, options=options)
#hallOfFame = EquationSearch(inputs_lay2[4, :]', activations_lay2[8, :]', niterations=100, options=options)
#hallOfFame = EquationSearch(inputs_lay2[5, :]', activations_lay2[9, :]', niterations=100, options=options)
#hallOfFame = EquationSearch(inputs_lay2[5, :]', activations_lay2[10, :]', niterations=100, options=options)
#hallOfFame = EquationSearch(inputs_lay2[6, :]', activations_lay2[11, :]', niterations=100, options=options)
#hallOfFame = EquationSearch(inputs_lay2[6, :]', activations_lay2[12, :]', niterations=100, options=options)
