...
Random.seed!(rng, 0)
tspan = Float32.([0.0, 50])
tspan_train=Float32.([0.0, 20])
u0 = Float32.([0.1, 0.5])
p_ = Float32[1.5, 1, 1, 3]
prob = ODEProblem(CSTR!, Float64.(u0), tspan, Float64.(p_))
...
function plotter(l, p_list, epoch)
    ...
    file = matopen(dir*add_path*"checkpoints/"*fname*"_results_MLP_steady.mat", "w")
